...
@Retention(RetentionPolicy.RUNTIME) // 프로그램 실행중에 항상 적용 (수명)
...
public @interface ValidEnum {
    ...
    // --- 필수 3요소 (규칙처럼 사용) ---
    String message() default "Invalid value"; // 검증 실패 시 보여줄 기본 메시지
    Class<?>[] groups() default {}; // 상황별 검증 그룹 (거의 초기에는 {} 빈 값으로 둠)
    Class<? extends Payload>[] payload() default {}; // 메타 데이터 (거의 {} 빈 값으로 둠)
}
